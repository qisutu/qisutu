...
sub OptionSearch {
    ...
    if($FieldKey=~m{\Adynamic:(\d+)\z}){$SQL='SELECT option_key AS id,option_value AS label FROM ticket_dynamic_field_option WHERE field_id=? AND (?="" OR option_value LIKE ? ESCAPE "\\\\") ORDER BY sort_order,option_value,id LIMIT 50';@Bind=($1,$Search,$Like);}
