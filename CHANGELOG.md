...
Existing API accesses can now be edited, deactivated and reactivated from the administration. Editing covers the label, Qisutu user, scopes, validity, allowed IP addresses and request limit while retaining the existing API key and activation state. Expiry is preserved by default; administrators can explicitly extend it or remove the expiry date. Expired accesses require a validity extension before reactivation, and inactive or system accounts cannot be reactivated for API use. API administration writes require POST and use the existing CSRF protection. The access list and request log now use padded, wrapping tables with clearer status indicators and actions; permission checkboxes are stacked consistently. New controls and messages are translated into all eleven interface languages. No database schema change is required for these API administration improvements.

The FAQ insertion dialog in new and existing agent tickets now stays within the visible window. Search results and article previews scroll inside the dialog, while the header and insertion/cancel buttons remain visible. On small or short screens, the dialog body scrolls to keep the text and attachment options accessible. Opening the dialog also locks the ticket workspace against background scrolling.
